...
function index()
	entry({"admin", "guide"}, call("action_guide"), _("Quick Guide"), 11)
end
...
if luci.http.formvalue("cbid.network.wireless.channel") then
       wireless_channel = luci.http.formvalue("cbid.network.wireless.channel")
	   uci2:set("wireless", wifi_dev, "channel", wireless_channel)	   
end
		
if luci.http.formvalue("cbid.network.wireless.ssid") then
       wireless_ssid = luci.http.formvalue("cbid.network.wireless.ssid") 
	   uci2:set("wireless", "cfg033579", "ssid", wireless_ssid)
end


if luci.http.formvalue("cbid.network.wireless.key") then
       	        wireless_key = luci.http.formvalue("cbid.network.wireless.key")
		uci2:set("wireless", "cfg033579", "encryption", "psk2")
		uci2:set("wireless", "cfg033579", "key", wireless_key)
end
		if  wireless_key == "" then
		uci2:set("wireless", "cfg033579", "encryption","none")	
		uci2:set("wireless", "cfg033579", "key","")
		else
...
elseif step == 4 then
 uci = luci.model.uci.cursor()
		...
		uci2:load("wireless")
		uci2:commit("wireless")
		if not network_wan_proto == "dhcp" then
